            ...
            if(orderOtherResult.isStatus() == true){
                ...
                if(order.getStatus() == OrderStatus.NOTPAID.getCode()
                    ...
                    if(handleVoucherStatus == true){
                        logger.info("[Handle Voucher] Success.");
                    }else{
                        throw new Exception("Fail.");
//                        logger.info("[Handle Voucher] Fail.");
//                        CancelOrderResult result = new CancelOrderResult();
//                        result.setStatus(false);
//                        result.setMessage("Voucher Handle Fail");
//                        logger.info("[Cancel Order Service][Cancel Order] Voucher Handle Fail.");
//                        return result;
                    }
    ...
    private boolean handleVoucher(String orderId){
       ...
       for(int i = 0; i < maxRetry; i++){
           ...
           logger.info("[Handle Voucher] Message:" + result);

           if(result.contains("Success")){
               logger.info("Success");
               return true;
           }else{
               logger.info("Contain Retry");
               try {
                   Thread.sleep(5000);
               }catch(Exception e){
                   e.printStackTrace();
