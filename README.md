...
6. You will receive a timeout. (Comment by PU: The first time after some waiting, there appears a message with "Something wrong". When canceling the same ticket a second time, 
the UI almost immediately states: "Not permitted". -> The behaviour between invocations changes)