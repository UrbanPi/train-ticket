...
public class PreserveServiceImpl implements PreserveService{
    private final static org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(PreserveServiceImpl.class);
    ...
    public OrderTicketsResult preserve(OrderTicketsInfo oti,String accountId,String loginToken){
        ...
        if(tokenResult.isStatus() == true){
            logger.info("[Preserve Service][Verify Login] Success");
            //1.黄牛检测
            ...
            logger.info("[Preserve Service] [Step 2] Find contacts");
            GetContactsInfo gci = new GetContactsInfo();
            logger.info("[Preserve Service] [Step 2] Contacts Id:" + oti.getContactsId());
            ...
            if(gcr.isStatus() == false){
                logger.info("[Preserve Service][Get Contacts] Fail." + gcr.getMessage());
                otr.setStatus(false);
                otr.setMessage(gcr.getMessage());
                otr.setOrder(null);
                return otr;
            }
            logger.info("[Preserve Service][Step 2] Complete");
            //3.查询座位余票信息和车次的详情
            logger.info("[Preserve Service] [Step 3] Check tickets num");
            ...
            logger.info("[Preserve Service] [Step 3] TripId:" + oti.getTripId());
            GetTripAllDetailResult gtdr = getTripAllDetailInformation(gtdi);
            if(gtdr.isStatus() == false){
                logger.info("[Preserve Service][Search For Trip Detail Information] " + gcr.getMessage());
                otr.setStatus(false);
                otr.setMessage(gcr.getMessage());
                otr.setOrder(null);
                return otr;
                ...
                if(oti.getSeatType() == SeatClass.FIRSTCLASS.getCode()){
                    if(tripResponse.getConfortClass() == 0){
                        logger.info("[Preserve Service][Check seat is enough] " + gcr.getMessage());
                        otr.setStatus(false);
            ...
            logger.info("[Preserve Service] [Step 3] Tickets Enough");
            //4.下达订单请求 设置order的各个信息
            logger.info("[Preserve Service] [Step 4] Do Order");
            ...
            logger.info("[Preserve Service][Order] Order Travel Date:" + oti.getDate().toString());
...
//                order.setSeatNumber("SecondClass-" + secondClassRemainNum);
            ...
            logger.info("[Preserve Service][Order Price] Price is: " + order.getPrice());
            ...
            if(cor.isStatus() == false){
                logger.info("[Preserve Service][Create Order Fail] Create Order Fail." +
            ...
            logger.info("[Preserve Service] [Step 4] Do Order Complete");
            otr.setStatus(true);
            ...
            if(oti.getAssurance() == 0){
                ...
                if(addAssuranceResult.isStatus() == true){
                    logger.info("[Preserve Service][Step 5] Buy Assurance Success");
                }else{
            ...
            //6.增加订餐
//            logger.info("[Food Service]!!!!!!!!!!!!!!!foodstorename=" + oti.getStationName()+"   "+oti.getStoreName());
            if(oti.getFoodType() != 0){
                ...
                if(oti.getFoodType() == 2){
                    ...
                    logger.info("[Food Service]!!!!!!!!!!!!!!!foodstore=" + afoi.getFoodType() + "   " + afoi.getStationName()+"   "+afoi.getStoreName());
                }
            ...
            if(null != oti.getConsigneeName() && !"".equals(oti.getConsigneeName())){
                ...
                if(icresult.isStatus()){
                    ...
                    logger.info("[Preserve Service][Step 7] Consign Fail.");
            ...
            logger.info("[Preserve Service][Verify Login] Fail");
            otr.setStatus(false);
    ...
    private AddAssuranceResult addAssuranceForOrder(int assuranceType,String orderId){
        logger.info("[Preserve Service][Add Assurance For Order]");
    ...
    private CheckResult checkSecurity(CheckInfo info){
        logger.info("[Preserve Service][Check Security] Checking....");
    ...
    private VerifyResult verifySsoLogin(String loginToken){
        logger.info("[Preserve Service][Verify Login] Verifying....");
    ...
    private GetTripAllDetailResult getTripAllDetailInformation(GetTripAllDetailInfo gtdi){
        logger.info("[Preserve Service][Get Trip All Detail Information] Getting....");
    ...
    private GetContactsResult getContactsById(GetContactsInfo gci){
        logger.info("[Preserve Service][Get Contacts By Id] Getting....");
    ...
    private CreateOrderResult createOrder(CreateOrderInfo coi){
        logger.info("[Preserve Service][Get Contacts By Id] Creating....");
    ...
    private AddFoodOrderResult createFoodOrder(AddFoodOrderInfo afi){
        logger.info("[Preserve Service][Add food Order] Creating....");
    ...
    private InsertConsignRecordResult createConsign(ConsignRequest cr){
        logger.info("[Preserve Service][Add Condign] Creating....");
