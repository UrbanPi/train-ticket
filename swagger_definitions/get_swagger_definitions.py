...
available_versions = ["error-f4"]

version_groups = {"master_grp": ["master"], "error_grp": ["error-f4"]}

services = [{"name": "ts-admin-basic-info-service", "port": 30030,
             "versions": ["master_grp", "error_grp"]},
...
def retrieve_and_save(service: dict):
    ...
    try:
        ...
        if isinstance(error.reason, socket.timeout):
            return 'Timeout Error for {} : This is a request with SSL which is only used in special scenarios of TrainTicket. Maybe you started the wrong file?\n URL{}'.format(
...
if __name__ == '__main__':
    ...
    if not version_argument:
        print("No version selected defaulting to version \"error-f4\"", file=sys.stderr)
        version_argument = "error-f4"
    else:
