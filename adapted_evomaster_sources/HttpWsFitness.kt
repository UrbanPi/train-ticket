...
abstract class HttpWsFitness<T>: ApiWsFitness<T>() where T : Individual {
    ...
    protected fun getTokens(ind: T): Map<String, String>{
        ...
        for(tl in tokensLogin){
            ...
            val response = try {
                client.target(tl.endpoint)// Pirmin Urbanke: Changed to interpret endpoint as the full URL. Allows to use a different service for auth.
                        .request()
        ...
        for (cl in cookieLogins) {
            ...
            val response = try {
                client.target(cl.getUrl(baseUrl))
                        .request()
                        .cookie("YsbCaptcha", "some-value") // Pirmin Urbanke: Add cookie for Trainticket error branches
            ...
            if (response.statusInfo.family != Response.Status.Family.SUCCESSFUL) {
                ...
                */
                if (response.statusInfo.family == Response.Status.Family.REDIRECTION) {
                    ...
                    log.warn("Login request failed with status ${response.status}. URL: ${response.location} or ${cl.getUrl(baseUrl)}")
                    continue
                }
            }
